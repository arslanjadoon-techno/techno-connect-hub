...
export class AuthService {
  login(email: string, password: string) {
    return http.post<{ token?: string; user?: BackendUser; requires2FA?: boolean; twoFactorRequired?: boolean }>(
  ...
  resetPassword(email: string, otp: string, newPassword: string, confirmPassword: string) {
    return http.post<null>(AUTH_PATHS.resetPassword,
      { email, otp, newPassword, confirmPassword }, false);
  }
  ...
  twoFaLoginVerify(email: string, code: string) {
    return http.post<{ token: string; user: BackendUser }>(
      AUTH_PATHS.twoFaLoginVerify, { email, code }, false);
  }
}
