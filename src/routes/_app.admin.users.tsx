...
function mapBackendToFrontendUser(bu: BackendUser): User {
  ...
  return {
    ...
    email: bu.email,
    role: bu.role.toLowerCase() as Role, // Sync casing differences if any
    department: bu.department as Department || "Engineering",
    avatarColor: "#0d7a5f", // Default view fallback
...
function UsersPage() {
  ...
  // 📥 1. REAL API: Fetch all users on mount
  useEffect(() => {
    let isMounted = true;
    ...
    async function fetchUsers() {
      ...
      try {
        ...
        if (response.success && Array.isArray(response.data) && isMounted) {
          const mappedUsers = response.data.map(mapBackendToFrontendUser);
          set("users", mappedUsers);
        }
      } catch (err) {
        if (isMounted) {
          toast.error((err as Error).message || "Failed to fetch real users data");
        }
  ...
  // 🗑️ 2. REAL API: Delete implementation
  const handleDelete = async (u: User) => {
    const numericId = Number(u.id);
    if (Number.isNaN(numericId)) {
...
function UserForm({ initial, onSaved }: { initial: User | null; onSaved: (u: User) => void }) {
  ...
  const submit = async () => {
    ...
    try {
      if (initial) {
        // ✏️ 3. REAL API: Update implementation
        const numericId = Number(initial.id);
        const response = await usersApi.update({
          ...
          email: email.trim(),
          role: role,
          department: departmentRequired ? department : "all", // "all" string handling as per your Spring check
        });
        ...
        // ➕ 4. REAL API: Add implementation
        const response = await usersApi.add({
          fullName: fullName.trim(),
          email: email.trim(),
          password,
          role,
          department: departmentRequired ? department : "all",
        });
