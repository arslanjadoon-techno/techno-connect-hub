  ...
  const fetchHouses = async (
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.message || "Failed to load houses");
        lastFetchedKey.current = "";
      }
    } catch (err: any) {
  ...
  const handleDelete = async (s: House) => {
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.message || "Could not delete house");
      }
    } catch (err: any) {
  ...
  if (loading && houses.length === 0) {
    return (
      <div className="flex h-[50vh] w-full flex-col items-center justify-center gap-2 text-muted-foreground">
        ...
        <p className="text-sm font-medium">Loading Houses Management Portal...</p>
      </div>
    );
  }
