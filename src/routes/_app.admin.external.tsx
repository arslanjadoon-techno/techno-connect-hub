...
function ExternalPage() {
  ...
  const handleSave = async (
    ...
    formData: { name: string; phone: string; marketId: number; address: string; workNature: string },
    close: () => void
  ) => {
    try {
      setActionLoading(true);
      ...
      <div className="w-full border-0 shadow-none bg-transparent [&_input]:bg-white dark:[&_input]:bg-zinc-950 [&_button.w-\[180px\]]:bg-white dark:[&_button.w-\[180px\]]:bg-zinc-950 [&_thead]:bg-zinc-200 dark:[&_thead]:bg-zinc-800 [&_thead]:border-b-2 [&_thead]:border-border [&_th]:font-bold [&_th]:text-zinc-900 dark:[&_th]:text-zinc-100 [&_th]:h-12 [&_tbody_tr]:bg-background [&_tbody_tr]:even:bg-zinc-50/50 dark:[&_tbody_tr]:even:bg-zinc-900/30 [&_tbody_tr]:hover:bg-muted/40 [&_th:last-child]:text-right [&_th:last-child]:pr-10 [&_td:last-child]:text-right [&_td[colspan]]:text-center [&_td[colspan]]:font-medium">
        ...
        <div className="[&_.flex-col]:flex-row [&_.flex-col]:items-center [&_.flex-col]:justify-between [&_.max-w-sm]:order-last [&_.max-w-sm]:ml-auto">
          <CrudPage<ExternalVendor>
            ...
            columns={[
              { key: "name", header: "Name", accessor: (v) => <div className="py-2 text-left font-medium">{v.name}</div>, searchValue: (v) => v.name },
              { key: "phone", header: "Phone", accessor: (v) => <div className="py-2 text-left text-muted-foreground">{v.phone}</div>, searchValue: (v) => v.phone },
              { key: "market", header: "Market", accessor: (v) => <div className="py-2 text-left text-zinc-700 dark:text-zinc-300 font-medium">{getMarketName(v.marketId)}</div>, searchValue: (v) => getMarketName(v.marketId) },
              { key: "address", header: "Address", accessor: (v) => <div className="py-2 text-left text-muted-foreground max-w-[200px] truncate">{v.address || "—"}</div>, searchValue: (v) => v.address },
              { key: "nature", header: "Nature of work", accessor: (v) => <div className="py-2 text-left text-zinc-600 dark:text-zinc-400 font-medium">{v.workNature}</div>, searchValue: (v) => v.workNature },
            ]}
...
interface VendorFormProps {
  ...
  markets: Market[];
  isSaving: boolean;
  onSave: (data: { name: string; phone: string; marketId: number; address: string; workNature: string }) => void;
}
...
function VendorForm({ initial, markets, isSaving, onSave }: VendorFormProps) {
  ...
  const filteredMarkets = useMemo(() => markets.filter((m) => m.name.toLowerCase().includes(marketSearch.toLowerCase())), [markets, marketSearch]);

  return (
    <div className="space-y-4">
      <div className="space-y-1.5">
        <Label>Vendor Name</Label>
        <Input value={name} disabled={isSaving} onChange={(e) => setName(e.target.value)} placeholder="e.g. Acme Tech Repairs" />
      </div>

      <div className="space-y-1.5">
        <Label>Phone Number</Label>
        <Input value={phone} disabled={isSaving} onChange={(e) => setPhone(e.target.value)} placeholder="e.g. +1 (555) 019-2834" />
      </div>
      ...
      <div className="space-y-1.5">
        <Label>Address</Label>
        <Input value={address} disabled={isSaving} onChange={(e) => setAddress(e.target.value)} placeholder="Physical workshop location" />
      </div>

      <div className="space-y-1.5">
        <Label>Nature of Work</Label>
        <Input value={workNature} disabled={isSaving} onChange={(e) => setWorkNature(e.target.value)} placeholder="e.g. HVAC repair, Commercial Plumbing" />
      </div>
