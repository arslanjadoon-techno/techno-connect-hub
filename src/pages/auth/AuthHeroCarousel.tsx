...
export function AuthHeroCarousel() {
  ...
  return (
    <div className="relative z-10 w-full max-w-lg flex flex-col justify-center my-auto min-h-[260px] lg:min-h-[280px] xl:min-h-[340px]">
      <AnimatePresence mode="wait">
        {currentSlide === 0 ? (
          ...
          >
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-[11px] sm:text-xs font-semibold text-white/90">
              <Sparkles className="w-3.5 h-3.5 text-purple-200" />
              <span>Unified MIS Ecosystem</span>
            </div>

            <h1 className="font-display text-2xl sm:text-3xl lg:text-[28px] xl:text-[36px] 2xl:text-[40px] font-semibold leading-[1.2] tracking-tight text-white">
              One platform for every portal, every team, every decision.
            </h1>

            <p className="text-white/85 text-xs sm:text-sm xl:text-base leading-relaxed font-normal">
      ...
      <div className="mt-4 xl:mt-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] sm:text-[11px] text-white/70 ml-2 font-medium tracking-wide">
            {currentSlide === 0 ? "Overview" : "Live Portals Network"}
          </span>
        </div>

        {/* Prev / Next Manual Navigation */}
        <div className="flex items-center gap-1 bg-white/10 backdrop-blur-sm rounded-full p-0.5 border border-white/15">
          <button
            type="button"
            onClick={handlePrev}
          ...
          <span className="text-[10px] sm:text-[11px] text-white/75 font-semibold px-1 select-none">
