...
export default function StarRankerPage() {
  ...
  const topThree = performers.filter((p) => p.rank <= 3);

  // Bottom 3 in stacked lines/rows (Normal theme)
  const bottomThree = performers.filter((p) => p.rank > 3 && p.rank <= 6);
          ...
          <div className="relative my-2">
            <div className="relative p-1.5 rounded-full bg-gradient-to-tr from-slate-300 via-sky-100 to-indigo-300 dark:from-slate-600 dark:via-sky-500 dark:to-indigo-400 shadow-lg">
              {!imgFailed && performer.photo ? (
                ...
                <div className="h-32 w-32 sm:h-36 sm:w-36 rounded-full flex items-center justify-center font-display text-3xl font-bold bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-200 border-2 border-white dark:border-slate-900 shadow-md">
          ...
          <div className="relative my-2">
            <div className="relative p-1.5 rounded-full bg-gradient-to-tr from-amber-500 via-yellow-300 to-amber-400 shadow-lg">
              {!imgFailed && performer.photo ? (
                ...
                <div className="h-32 w-32 sm:h-36 sm:w-36 rounded-full flex items-center justify-center font-display text-3xl font-bold bg-amber-200 text-amber-900 dark:bg-amber-900 dark:text-amber-100 border-2 border-white dark:border-slate-900 shadow-md">
                  {getInitials(performer.name)}
                </div>
              )}
            </div>
        ...
        <div className="relative my-2">
          <div className="relative p-1.5 rounded-full bg-gradient-to-tr from-zinc-400 via-slate-200 to-zinc-300 shadow-lg">
            {!imgFailed && performer.photo ? (
              ...
              <div className="h-32 w-32 sm:h-36 sm:w-36 rounded-full flex items-center justify-center font-display text-3xl font-bold bg-zinc-200 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200 border-2 border-white dark:border-slate-900 shadow-md">
                {getInitials(performer.name)}
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative shrink-0">
              {!imgFailed && performer.photo ? (
                ...
                <div className="h-14 w-14 rounded-xl flex items-center justify-center font-display text-sm font-bold bg-muted text-muted-foreground border border-border/80 shadow-xs">
                  {getInitials(performer.name)}
                </div>
              )}
            </div>
